...
const routes = {
  "GET /state": () => state(),
  "POST /pots": (b) => run("pot deploy", ["--deadline", b.deadline, ...(b.label ? ["--label", b.label] : []), ...(b.question ? ["--question", b.question] : []), ...(b.account ? ["--account", b.account] : []), ...(b.pattern ? ["--pattern", b.pattern] : [])]),
