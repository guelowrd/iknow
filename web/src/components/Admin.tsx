...
export function Admin({ markets, onChanged }: { markets: Market[]; onChanged: () => void }) {
  ...
  const [form, setForm] = useState({ deadline: "", label: "", question: "", account: "", pattern: "" });
  ...
  const pots = server?.pots ?? markets.map((m) => ({ id: m.id, label: m.label, question: m.question, deadlineMs: m.deadlineMs, lockHeight: m.lockHeight, account: "", pattern: "", status: { yesUnits: m.yes, noUnits: m.no, outcome: OUTCOME[m.outcome], vault: "", waitingNotes: 0 } }));
          ...
          <>
            ...
            <div className="form">
              ...
              <button className="btn wide go" disabled={!!busy || !form.deadline} onClick={() => act("deploy", "/pots", {
                deadline: new Date(form.deadline + "Z").toISOString(), label: form.label || undefined, question: form.question || undefined,
